...
;;;; Originally guile's ice-9 scm-style-repl, but, lightly patched

(define-module (sandy scm-style-repl)
  #:use-module (sandy save-stack)
...
(define* (scm-style-repl module #:optional (sandbox #f))
  (letrec (
           (start-gc-rt #f)
           (start-rt #f)
           ...
           (-eval (lambda (sourc)
                    ...
                    (let ((val (start-stack 'repl-stack
                                            ...
                                            ;; continues to work.
					    (if sandbox
						(eval-in-sandbox sourc #:module module #:sever-module? #f)
						(eval sourc module)
...
(define (handle-system-error key . args)
  (let ((cep (current-error-port)))
    ...
    (apply display-error (stack-ref (fluid-ref the-last-stack) 2) cep args)
